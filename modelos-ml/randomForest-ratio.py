...
full_data['goodput_retrans_ratio'] = full_data['goodThroughput'] / (full_data['retransmissions'] + 1)  # +1 para evitar división por cero
...
columns_to_normalize = full_data.columns.difference(['w', 'c', 'tam_red', 'num_stas', 'goodput_retrans_ratio'])
...
X = full_data.drop(columns=['goodput_retrans_ratio'])  #El ratio goodput-retransmissions es la variable objetivo
y = full_data['goodput_retrans_ratio']
...
model_filename = 'random_forest_model_ratio.joblib'
scaler_filename = 'scaler_rf_ratio.joblib'
...
plt.title('Resultados de la Búsqueda Aleatoria para RandomForest optimizando el ratio')
plt.legend()
plt.grid(True)
plt.show()
