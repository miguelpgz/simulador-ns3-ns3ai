...
int main(int argc, char *argv[])
{
  ...
  positionAlloc->Add (Vector (20, 0, 0));  // Añade una pos para el segundo nodo
  ...
  stas = NodeContainer(nodes.Get (1));
  ...
  Ipv4InterfaceContainer apInterface = address.Assign (apDev);
  Ipv4InterfaceContainer staInterfaces = address.Assign (staDevs);

  // Get source address
  auto ipv4 = stas.Get (0)->GetObject<Ipv4> (); //Get ip objetct from transmiter
  auto source = ipv4->GetAddress (1, 0).GetLocal ();
  ipv4 = ap.Get (0)->GetObject<Ipv4> ();
  auto destination = ipv4->GetAddress (1, 0).GetLocal ();
  ...
  ApplicationContainer apps = onoff.Install (stas.Get (0)); // Luego tendrás que cambiar esto para que sea un bucle que instale en todas las estaciones
